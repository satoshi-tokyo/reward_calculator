...
def main():
    ...
    if args.db:
        if is_current_epoch:
            ...
            with open(os.path.join(json_folder, epoch_data_json), "w") as f:
                json.dump(calc_obj.params, f)
        else:
            # e{epoch#}.json for historical data of the epoch
            epoch_data_json = "e" + str(calc_obj.params["epoch"]) + ".json"
        ...
        _epochs = ep.epoch_stake_timing(6)
        for e in range(1, 5):
            _epochs.append(ep.epoch_stake_timing_in_future(e))
