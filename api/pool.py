...
class Pool(object):
    ...
    def pool_historical_data(self, epoch="", ext_pool_id=""):
        ...
        while pages:
            if ext_pool_id:
                _url = f"{self.mainnet_url}/pools/{ext_pool_id}/history/?page={page_num}"
            else:
                _url = f"{self.mainnet_url}/pools/{POOL_ID}/history/?page={page_num}"
            _response = requests.get(_url, headers=HEADERS)

            if not json.loads(_response.text):
                pages = False
            historical_pool_data.extend(json.loads(_response.text))
            page_num += 1
        if epoch:
            for data in historical_pool_data:
                if data["epoch"] == epoch:
                    return data
        else:
